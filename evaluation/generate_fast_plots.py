...
def plot_1_cumulative_rewards_from_json(all_results, random_rewards):
    """Plot 1: Cumulative Rewards - Using pre-computed data"""
    print("\n[1/6] Cumulative Rewards...")
    ...
    for algo in ['PPO', 'DQN', 'A2C', 'REINFORCE']:
        if algo in all_results and len(all_results[algo]) > 0:
            ...
            log_path = Path(f"logs/{algo.lower()}/{best['config_name']}")
            monitor_files = list(log_path.glob("**/monitor.csv"))
            ...
            if monitor_files:
                try:
                    ...
                    if 'r' in df.columns:
                        ...
                        ax_comp.plot(episodes, smoothed, color=COLORS[algo], linewidth=3.5, 
                                    label=f"{algo} ({best['mean_eval_reward']:.0f}±{best['std_eval_reward']:.0f})",
                                    alpha=0.85, marker='s', markersize=6, markevery=len(smoothed)//10)
                        continue
                except:
                    pass
