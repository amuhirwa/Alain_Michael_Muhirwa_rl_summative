...
@njit
def update_panic_jit(x, y, panic, alive, N, PANIC_TRIGGER, PANIC_INC, PANIC_DEC, SPREAD_RAD):
    """JIT-accelerated panic update with density-based increase/decrease"""
    for i in range(N):
        if not alive[i]:
            continue
        local_count = 0
        ...
        # Panic increases with density, decreases otherwise
        if local_count > PANIC_TRIGGER:
            panic[i] = min(1.0, panic[i] + PANIC_INC)
        else:
            panic[i] = max(0.0, panic[i] - PANIC_DEC)
        
        # Panic spreads from highly panicked agents
        if panic[i] > 0.5:
            for j in range(N):
...
class EnhancedCrowdControlEnvFast(gym.Env):
    ...
    def __init__(self, render_mode=None, crowd_arrival_pattern="rush",
        ...
        self.entrances = [(2, 2), (2, 17), (17, 2), (17, 17), (10, 2)]
        self.timestep = 0
    ...
    def _spawn_arrivals(self):
        ...
        if self.crowd_arrival_pattern == "rush":
            rate = 6 * np.exp(-((p - self.rush_peak_time) ** 2) / 0.05)
        elif self.crowd_arrival_pattern == "steady":
            rate = 2 if p < 0.8 else 0
        elif self.crowd_arrival_pattern == "evacuation":
            rate = 10 if p < 0.1 else 0
        else:
            rate = 2
    ...
    def _update_panic(self):
        """Controllable panic: increases with density, decreases otherwise"""
        update_panic_jit(
            self.x, self.y, self.panic, self.alive, self.MAX_AGENTS,
            self.PANIC_TRIGGER_DENSITY, self.PANIC_INCREASE_RATE,
            self.PANIC_DECREASE_RATE, self.PANIC_SPREAD_RADIUS
        )
